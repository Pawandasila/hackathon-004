...
export default function ListingDetailPage() {
  ...
  const handleSendMessage = () => {
    if (currentUser && listing && currentUser._id === listing.sellerId) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20 pb-8">
      <div className="container mx-auto px-4 max-w-8xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="flex gap-2 pt-4">
                      {/* Only show Send Message if user is not the seller */}
                      {currentUser &&
                        listing &&
                        currentUser._id !== listing.sellerId && (
                          <Button
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                  <div>
                    ...
                    <p className="font-medium">
                      {formatTime(listing._creationTime)}
                    </p>
                  </div>
                  <div>
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-gray-600">
                  See how this compares to other platforms
                </p>
              </CardHeader>
                ...
                {platformPrices.length > 0 ? (
                  platformPrices.map((platformData) => (
                    <div
                      ...
                      className={`flex justify-between items-center p-3 rounded-lg border ${platformData.isAvailable ? "bg-white" : "bg-gray-50"}`}
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-lg">{platformData.logo}</span>
                        <div>
                          <span
                            className={`text-sm font-medium ${!platformData.isAvailable ? "text-gray-400" : ""}`}
                          >
                      ...
                      <div className="text-right">
                        {platformData.isAvailable ? (
                          <>
                            ...
                            {platformData.savingsFromUser &&
                              platformData.savingsFromUser > 0 && (
                  ...
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-center">
                    ...
                    <p className="text-xs text-blue-600">
                      Our AI is learning market prices to provide you with the
                      best comparison insights!
                    </p>
                  </div>
                )}
                  ...
                  <p className="text-sm text-green-700 font-medium">
                    {(() => {
                      const availablePlatforms =
                        platformComparison?.availablePlatforms || 0;
                      const mostExpensivePrice =
                        platformComparison?.mostExpensivePrice;
                      ...
                      if (
                        !mostExpensivePrice ||
                        mostExpensivePrice <= listing.price
                      ) {
                        return "🏆 Great deal! This is already competitively priced among local sellers.";
                      }

                      const savings = Math.round(
                        ((mostExpensivePrice - listing.price) / listing.price) *
                          100
                      );
            ...
            {similarListings && similarListings.length > 0 ? (
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-600">
                    Similar listings for {masterItem?.name}
                  </p>
                </CardHeader>
                <CardContent className="space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="font-medium text-sm">
                            {similarListing.seller?.name}
                          </p>
                          <p className="text-xs text-gray-500">
                            {similarListing.quantity} {similarListing.unit} •{" "}
                            {similarListing.condition}
                          </p>
                        </div>
                        <div className="text-right">
                          <span className="font-semibold">
                ...
                <CardContent>
                  <div className="text-center p-4 bg-purple-50 border border-purple-200 rounded-lg">
                    ...
                    <p className="text-xs text-purple-600">
                      You're the first to discover this amazing local find. Our
                      community of sellers is growing daily!
                    </p>
                  </div>
                </CardContent>
              </Card>
        ...
        {/* Chat Dialog - Only show if user is not the seller */}
        {listing &&
          masterItem &&
          listing.seller &&
          currentUser &&
